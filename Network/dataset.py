...
# add for our data(training and validation)
# 我们的数据结构是
# dataset-----train----------groundtruth
#                  ----------input
#             validation-----groundtruth
#                       -----input
class SIDDValData2(Dataset):
    def __init__(self, path):

        val_data_dict = loadmat(os.path.join(path, 'validation','ValidationNoisyBlocksSrgb.mat'))
        val_data_noisy = val_data_dict['ValidationNoisyBlocksSrgb']
        val_data_dict = loadmat(os.path.join(path,'validation','ValidationGtBlocksSrgb.mat'))
        val_data_gt = val_data_dict['ValidationGtBlocksSrgb']
        self.num_block = 1
        self.num_img, h_, w_, c_ = val_data_gt.shape
        self.val_data_noisy = val_data_noisy
        self.val_data_gt = val_data_gt
...
# add for our data(testing)
# 我们的数据结构是
# dataset-----test
class SIDDValData3(Dataset):
    def __init__(self, path):

        val_data_dict = loadmat(os.path.join(path,'InputBlocksSrgb.mat'))
        val_data_noisy = val_data_dict['InputBlocksSrgb']
        self.num_block = 1
        self.num_img, h_, w_, c_ = val_data_noisy.shape
        self.val_data_noisy = val_data_noisy
        self.val_data_gt = val_data_noisy
